/**
 * CYX俱乐部 - Cloudflare Workers API (v3 - 实际数据库版本)
 *
 * 基于实际 D1 数据库结构：
...
// 订单号生成：CYX + MMDD + 商品码(3) + 验证码(4) + 序号(2)
...
async function hLogin(db, kv, req) {
  ...
  if (admin) {
    ...
    if (hash !== admin.password_hash) return err('密码错误', 401);
  ...
  if (role === 'employee') {
    ...
    const hash = await hashPwd(password);
    if (hash !== worker.password_hash) return err('密码错误', 401);
